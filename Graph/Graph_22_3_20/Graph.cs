...
namespace Graph_22_3_20
{
    public class Graph
    {
        private class Node //вложенный класс для скрытия данных и алгоритмов
        {
            ...
            public bool LowerOrNot(int N, long[,] ar)
            {
                for (int i = 0; i < Size; i++)
                {
        ...
        private City[] cities; // массив городов
        ...
        public Graph(string name) //конструктор внешнего класса
        {
            using (StreamReader file = new StreamReader(name))
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    string[] currentCity = Line.Split(" ");
                    c[i] = new City(currentCity[0], Int32.Parse(currentCity[1]), Int32.Parse(currentCity[2]));
                }
                for (int i = 0; i < n; i++)
                {
                    string line = file.ReadLine();
                    ...
                    for (int j = 0; j < n; j++)
                    {
                        if (int.Parse(mas[j]) == 1)
                        {
                            a[i, j] = City.Distance(c[i], c[j]);
                        }
                    }
                }
                cities = c;
        ...
        public void GraphEdit(int i, int j)
        {
            graph[i, j] = graph[j, i] = City.Distance(cities[i], cities[j]);
        }
        ...
        public int[] CheckForDistances(int N)
        {
            int[,] p;
            long[,] a = graph.Floyd(out p);
            int size = graph.Size;
            if (graph.LowerOrNot(N, a))
            {
                Console.WriteLine("Расстояния уже меньше, чем {0}", N);
                int[] res = { 0,0,0};
                return res;
            }
            for (int i = 0; i < size; i++)
            {
                for (int j = 0; j < size; j++)
                {
                    if (i == j || graph[i, j] == 0)
                    {
                        long distBeforeModify = a[i, j];
                        a[i, j] = a[j, i] = City.Distance(cities[i], cities[j]);
                        if (graph.LowerOrNot(N, a))
                        {
                            Console.WriteLine("\nНеобходимо соединить следующие пункты:");
                            cities[i].Show();
                            cities[j].Show();
                            Console.WriteLine();
                            int[] res1 = { 1, i, j };
                        ...
                        a[i, j] = a[j, i] = (int)distBeforeModify;
                    }
                }
            }
            Console.WriteLine("Нельзя построить одну дорогу так, чтобы расстояние между любыми городами было меньше {0}", N);
            int[] res2 = { 0, 0, 0 };
            return res2;
        }
    }
